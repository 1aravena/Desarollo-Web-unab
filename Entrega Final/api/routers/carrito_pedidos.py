"""
================================================================================
Router de Carrito y Pedidos - Pizzeria La Fornace
================================================================================

Este modulo contiene la logica de negocio principal del sistema de pedidos.
Maneja el ciclo completo: carrito -> resumen -> confirmacion -> seguimiento.

Endpoints de Carrito (/api/v1/carrito):
---------------------------------------
GET    /                      - Obtener carrito del usuario (B-03)
POST   /items                 - Agregar item al carrito (B-03)
PUT    /items/{id}            - Actualizar cantidad (B-03)
DELETE /items/{id}            - Eliminar item (B-03)
DELETE /                      - Vaciar carrito (B-03)

Endpoints de Pedidos (/api/v1/pedidos):
---------------------------------------
POST   /validar-direccion     - Validar cobertura de entrega (E-05)
GET    /resumen               - Vista previa antes de confirmar (B-04)
POST   /                      - Confirmar pedido (B-04, E-02)
GET    /                      - Historial de pedidos (B-02)
GET    /{id}                  - Detalle de un pedido (B-02)
PATCH  /{id}/estado           - Actualizar estado (cocina/admin)

Funcionalidades Clave:
----------------------
1. Validacion de stock en tiempo real (E-03)
2. Calculo de distancia con formula de Haversine (E-05)
3. Estimacion de tiempo de entrega (E-04)
4. Snapshots de items para historial inmutable
5. Envio de emails de confirmacion en background (B-12)
6. Cola de impresion para cocina (B-08)

Historias de Usuario:
---------------------
B-02: Historial de Pedidos
B-03: Carrito de Compras
B-04: Resumen de Pedido
B-08: Impresion de Comandas
B-12: Confirmacion por Email
E-02: Proceso de Pago
E-03: Control de Stock
E-04: Tiempo Estimado de Entrega (ETA)
E-05: Cobertura de Entrega

Dependencias:
-------------
- models.py: Carrito, CarritoItem, Pedido, Producto
- schemas.py: DTOs para entrada/salida
- auth.py: get_current_user para proteccion de endpoints
- services/email_service.py: Envio de notificaciones
================================================================================
"""
...
async def remove_carrito_item(
    ...
    result = await db.execute(
        select(CarritoItem)
        .options(joinedload(CarritoItem.extras))
        .join(Carrito)
        .where(
            CarritoItem.id == item_id,
            Carrito.user_id == current_user.id
...
async def clear_carrito(
    ...
    if carrito:
        ...
        # Limpiar extras de cada item primero
        for item in items:
            item.extras = []
        await db.flush()
        
        # Ahora eliminar los items
        for item in items:
            await db.delete(item)
...
async def create_pedido(
    ...
    # Obtener items del carrito con extras y tamaño cargados
    result = await db.execute(
        select(CarritoItem)
        .options(
            joinedload(CarritoItem.extras),
    ...
    for item in items:
        ...
        # Crear snapshot del item con tamaño y extras
        item_data = {
            "producto_id": producto.id,
            "nombre": producto.nombre,
        ...
        # Agregar información del tamaño si existe
        if item.tamanio:
            item_data["tamanio"] = {
                "id": item.tamanio.id,
                "nombre": item.tamanio.nombre,
                "precio_adicional": float(item.tamanio.precio_adicional)
            }
        else:
            item_data["tamanio"] = None
        
        # Agregar información de los extras si existen
        if item.extras:
            item_data["extras"] = [
                {
                    "id": extra.id,
    ...
    # Vaciar carrito - primero limpiar extras, luego items
    for item in items:
        # Limpiar la lista de extras del item
        item.extras = []
        await db.flush()
    ...
    # Enviar email de confirmación en segundo plano (B-12)
    async def enviar_email_confirmacion():
        try:
            contenido_html = EmailTemplates.confirmacion_pedido(
                ...
                eta_minutos=nuevo_pedido.eta_minutos or 45,
                items=items_snapshot,
                nombre_cliente=current_user.nombre or "Cliente"
            )
            ...
            if exito:
                logger.info(f"Email de confirmación enviado para pedido #{nuevo_pedido.id}")
            else:
                logger.warning(f"No se pudo enviar email de confirmación para pedido #{nuevo_pedido.id}")
...
async def get_pedidos(
    ...
    - Paginado
    """
    # Si es admin o cocinero, puede ver todos los pedidos
    if current_user.rol in ["admin", "administrador", "cocinero"]:
        query = select(Pedido)
    else:
...
@router_pedidos.patch("/{pedido_id}/estado", response_model=PedidoResponse)
async def update_pedido_estado(
    ...
    """
    Actualizar estado de un pedido (Solo para admin/cocinero)
    Estados válidos: pendiente, confirmado, en_preparacion, en_camino, entregado, anulado
    """
    # Verificar que el usuario es admin o cocinero
    if current_user.rol not in ["admin", "administrador", "cocinero"]:
        raise HTTPException(
    ...
    # Estados válidos
    estados_validos = ["pendiente", "confirmado", "en_preparacion", "en_camino", "entregado", "anulado", "cancelado"]
    ...
    # Buscar pedido con usuario
    result = await db.execute(
        select(Pedido)
        .options(joinedload(Pedido.usuario))
        .where(Pedido.id == pedido_id)
    )
    pedido = result.unique().scalar_one_or_none()
    ...
    # Guardar datos para el email antes del commit
    email_destino = pedido.usuario.email if pedido.usuario else None
    nombre_cliente = pedido.usuario.nombre if pedido.usuario else "Cliente"
    ...
    # Enviar email de notificación de cambio de estado (B-12)
    if email_destino and nuevo_estado in ["confirmado", "en_preparacion", "en_camino", "entregado"]:
        async def enviar_email_estado():
            try:
                contenido_html = EmailTemplates.cambio_estado_pedido(
                    pedido_id=pedido_id,
                    estado=nuevo_estado,
                    nombre_cliente=nombre_cliente
                )
                
                asunto = f"Tu pedido #{pedido_id} - {nuevo_estado.replace('_', ' ').title()}"
                ...
                logger.info(f"Email de cambio de estado enviado para pedido #{pedido_id}")
            except Exception as e:
                logger.error(f"Error enviando email de estado: {str(e)}")
        
        background_tasks.add_task(enviar_email_estado)
