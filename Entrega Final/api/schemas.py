...
class ProductoSimpleResponse(BaseModel):
    """Schema de respuesta de producto sin relaciones (para rankings/reportes)"""
    id: int
    nombre: str
    descripcion: Optional[str] = None
    precio: Decimal
...
class TamanioCreate(TamanioBase):
    """Schema para crear tamaño (B-06 - CRUD Productos)"""
    activo: bool = True


class TamanioUpdate(BaseModel):
    """Schema para actualizar tamaño (B-06 - CRUD Productos)"""
    nombre: Optional[str] = Field(None, max_length=50)
    precio_adicional: Optional[Decimal] = Field(None, ge=0, decimal_places=2)
    activo: Optional[bool] = None
...
class ExtraCreate(ExtraBase):
    ...
    disponible: bool = True
    activo: bool = True
    productos_ids: List[int] = Field(default_factory=list)


class ExtraUpdate(BaseModel):
    """Schema para actualizar extra"""
...
class ExtraSimpleResponse(ExtraBase):
    """Schema de extra sin productos_ids para uso en carrito/pedidos"""
    id: int
    disponible: bool
    activo: bool
...
class CarritoItemResponse(BaseModel):
    ...
    extras: List[ExtraSimpleResponse] = Field(default_factory=list)
    cantidad: int
    notas: Optional[str] = None
...
class PedidoEstadoUpdate(BaseModel):
    """Schema para actualizar estado de pedido (Cocina/Admin)"""
...
class CampaignInput(BaseModel):
    ...
    canal: str = Field(default="email", max_length=50)  # Solo email por ahora
